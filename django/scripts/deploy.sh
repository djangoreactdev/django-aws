...
# Collect ECS_GROUP_ID and public_SUBNET_ID for running migrations
...
public_SUBNET_ID=$(aws ec2 describe-subnets  --filters "Name=tag:Name,Values=prod-public-1" --query "Subnets[*][SubnetId]"  --output text)

echo "Running migration task..."
# Construct NETWORK_CONFIGURATON to run migtaion task 
NETWORK_CONFIGURATON="{\"awsvpcConfiguration\": {\"subnets\": [\"${public_SUBNET_ID}\"], \"securityGroups\": [\"${ECS_GROUP_ID}\"],\"assignPublicIp\": \"DISABLED\"}}"
